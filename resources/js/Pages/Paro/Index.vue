...
const selectAll = (event) => {
    if (event.target.checked === false) {
        ...
        props.fromController?.data.forEach((Paro) => {
            ...
            <div class="px-4 sm:px-0">
                <div class="rounded-lg overflow-hidden w-fit">
                    <PrimaryButton class="rounded-none" @click="data.createOpen = true"
                        v-if="can(['isAdmin','create Paro'])">
                        {{ lang().button.new }}
                    </PrimaryButton>
                    
                    
                    <Create v-if="can(['isAdmin','create Paro'])" :numberPermissions="props.numberPermissions" 
                        :titulos="titulos" :show="data.createOpen" @close="data.createOpen = false" :title="props.title"
                        :losSelect=props.losSelect />

                    <Edit v-if="can(['isAdmin','update Paro'])" :titulos="titulos"
                        :numberPermissions="props.numberPermissions" :show="data.editOpen" @close="data.editOpen = false"
                        :Paroa="data.Paroo" :title="props.title" :losSelect=props.losSelect />

                    <Delete v-if="can(['isAdmin','delete Paro'])" :numberPermissions="props.numberPermissions"
                        :show="data.deleteOpen" @close="data.deleteOpen = false" :Paroa="data.Paroo"
                        :title="props.title" />
                </div>
            </div>
                ...
                <div class="flex justify-between p-2">
                    <div class="flex space-x-2">
                        ...
                        <!-- <DangerButton @click="data.deleteBulkOpen = true"
                            v-show="data.selectedId.length != 0 && can(['isAdmin',delete Paro'])" class="px-3 py-1.5"
                    ...
                    <table v-if="props.total > 0" class="w-full">
                        ...
                        <tbody>
                            <tr v-for="(claseFromController, indexu) in props.fromController.data" :key="indexu"
                                ...
                                <td v-if="numberPermissions > 1" class="whitespace-nowrap py-4 w-12 px-2 sm:py-3">
                                    <div class="flex justify-center items-center">
                                        <div class="rounded-md overflow-hidden">
                                            <InfoButton v-show="can(['update Paro'])" type="button"
                                                @click="(data.editOpen = true), (data.Paroo = claseFromController)"
                                                class="px-2 py-1.5 rounded-none" v-tooltip="lang().tooltip.edit">
                                                <PencilIcon class="w-4 h-4" />
                                            </InfoButton>
                                            <DangerButton v-show="can(['isAdmin','delete Paro'])" type="button"
                                                @click="(data.deleteOpen = true), (data.Paroo = claseFromController)"
                                ...
                                <td class="whitespace-nowrap py-4 px-2 sm:py-3 text-center">{{ ++indexu }}
                                </td>
