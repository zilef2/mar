...
class ExcelController extends Controller
{
    ...
    // get
    public function subirexcelesVariable(string $importName) // just  a view
    {
        $permissions = Myhelp::EscribirEnLog($this, ' subir exceles GET');
        $numberPermissions = Myhelp::getPermissionToNumber($permissions);

        return Inertia::render('Subiendoxls/'.$importName, [
            'numUsuarios' => count(User::all()) - 1,
            'numOrdenes' => count(Ordenproduccion::all()),
            'numberPermissions' => $numberPermissions,
        ]);
    ...
    public function uploadExcelVariable(Request $request, string $importClass)
    {
        Myhelp::EscribirEnLog($this, get_called_class(), 'Empezo a importar', false);
        $countfilas = 0;
        $importer = null;
        ...
        try {
            if ($request->archivo1) {
                ...
                $fullClass = "App\\Imports\\{$importClass}";
                $importer = new $fullClass;
                Excel::import($importer, $request->archivo1);
                $countfilas = $importer->CountFilas;

                $MensajeWarning = $this->MensajeWar($importer);

                if ($MensajeWarning !== '') {
                    return back()->with('success', 'Registros nuevos: '.$countfilas)->with('warning', $MensajeWarning);
                }
            ...
            $fila = $importer?->larow[0] ?? 'desconocida';

            return back()->with('error', __('app.label.op_not_successfully').' Fila: '.$fila.' iteracion: '.$countfilas.' '.$th->getMessage().' L:'.$th->getLine());
        }
    }
