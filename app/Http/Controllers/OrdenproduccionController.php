...
class OrdenproduccionController extends Controller {
	...
	public function Filtros($request): Builder {
		...
		if ($request->has('search')) {
			$Ordenproduccions = $Ordenproduccions->where(function ($query) use ($request) {
				$query
					->where('pedido', 'LIKE', "%" . $request->search . "%")->orWhere('op', 'LIKE', "%" . $request->search . "%")//                    ->orWhere('identificacion', 'LIKE', "%" . $request->search . "%")
				;
			});
		}
	...
	public function PerPageAndPaginate($request, $Ordenproduccions) {
		...
		$paginated = new LengthAwarePaginator($Ordenproduccions->forPage($page, $perPage), $Ordenproduccions->count(), $perPage, $page, ['path' => request()->url()]);
	...
	public function update(Request $request, $id): RedirectResponse {
		...
		DB::commit();
		Myhelp::EscribirEnLog($this, 'UPDATE:Ordenproduccions EXITOSO', 'Ordenproduccion id:' . $Ordenproduccion->id . ' | ' . $Ordenproduccion->op, false);
		
		return back()->with('success', __('app.label.updated_successfully2', ['nombre' => $Ordenproduccion->op]));
	}
	...
	public function destroy($Ordenproduccionid) {
		...
		$Ordenproduccion = Ordenproduccion::findOrFail($Ordenproduccionid);
		$elnombre = $Ordenproduccion->nombre;
		
		try {
			$Ordenproduccion->delete();
			
			Myhelp::EscribirEnLog($this, 'DELETE:Ordenproduccions', 'Ordenproduccion id:' . $Ordenproduccion->id . ' | ' . $elnombre . ' borrado', false);
